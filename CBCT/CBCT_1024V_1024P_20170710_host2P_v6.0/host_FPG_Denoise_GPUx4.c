////*******************************************/////
void FGP_denoise_GPUx4_exact(float *h_A, float lambda, int Niter_denoise)
{    
    int SIZE = M*N*ZETA;             
    ...
    float *d_A = NULL;
	cudaMalloc((void**)&d_A, d_volume_size);    
    cudaMemcpy(d_A, h_A, d_volume_size, cudaMemcpyHostToDevice);        
...
//         printf(" - have done FGP denoise %d out of %d times: \n",k,Niter);   
    ...
    cudaMemcpy(d_volume, d_A, d_volume_size, cudaMemcpyDeviceToDevice);    
    F_operator_L_Pc_kernel<<<dimgrid_B, dimblock_B>>>(d_volume, d_r, d_s, d_t,lambda);            
    cudaMemcpy(h_A, d_volume, d_volume_size, cudaMemcpyDefault) ;    
...
void FGP_denoise_GPUx4_apprx(float *h_A, float lambda, int Niter_denoise)
{
    int SIZE = M*N*ZETA;               
...
void GP_denoise_GPUx4_fast(float *h_A, float lambda, int Niter_denoise)
{
    int SIZE = M*N*ZETA;                       
