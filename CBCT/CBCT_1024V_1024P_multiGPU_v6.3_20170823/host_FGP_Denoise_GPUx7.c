...

{
    ...
    checkCuda(cudaMalloc((void**)&d_r, d_r_array_size));
    checkCuda(cudaMemset(d_r, 0, d_r_array_size) );
	
    // s
	float *d_s = NULL;
	size_t d_s_array_size = sizeof(float)*(NO_Z)*(NO_Y+1)*(NO_X);
    checkCuda(cudaMalloc((void**)&d_s, d_s_array_size));
    checkCuda(cudaMemset(d_s, 0, d_s_array_size) );
    
    // t 
	float *d_t = NULL;
	size_t d_t_array_size = sizeof(float)*(NO_Z+1)*(NO_Y)*(NO_X);
    checkCuda(cudaMalloc((void**)&d_t, d_t_array_size));
    checkCuda(cudaMemset(d_t, 0, d_t_array_size) );
    
    // o 
	float *d_o = NULL;
	size_t d_o_array_size = sizeof(float)*(NO_Z)*(NO_Y)*(NO_X+1);
    checkCuda(cudaMalloc((void**)&d_o, d_o_array_size));
    checkCuda(cudaMemset(d_o, 0, d_o_array_size) );
	
    // p
	float *d_p = NULL;
	size_t d_p_array_size = sizeof(float)*(NO_Z)*(NO_Y+1)*(NO_X);
    checkCuda(cudaMalloc((void**)&d_p, d_p_array_size));
    checkCuda(cudaMemset(d_p, 0, d_p_array_size) );
    
    // q 
	float *d_q = NULL;
	size_t d_q_array_size = sizeof(float)*(NO_Z+1)*(NO_Y)*(NO_X);
    checkCuda(cudaMalloc((void**)&d_q, d_q_array_size));
    checkCuda(cudaMemset(d_q, 0, d_q_array_size) );
	
    // L
    float *d_L = NULL;
	size_t d_L_array_size = sizeof(float)*(NO_Z)*(NO_Y)*(NO_X);
    checkCuda(cudaMalloc((void**)&d_L, d_L_array_size));
    checkCuda(cudaMemset(d_L, 0, d_L_array_size) );
	...
	checkCuda(cudaMalloc((void**)&d_f_noise, d_volume_size));    
