...
{        
    ...
    {
       cudaSetDevice(i); 
       checkCuda(cudaMalloc((void**)&d_proj_sumLen_addr[i], size_proj_single));  
       cudaMemset(d_proj_sumLen_addr[i],0,size_proj_single);       
       checkCuda(cudaMalloc((void**)&d_proj_weightedLen_addr[i], size_proj_single)); 
       cudaMemset(d_proj_weightedLen_addr[i],0,size_proj_single);       
       checkCuda(cudaMalloc((void**)&d_proj_addr[i], size_proj_single)); 
       cudaMemset(d_proj_addr[i], 0, size_proj_single);
       checkCuda(cudaMalloc((void**)&d_volume_addr[i], size_volume_portion)); 
	...
	dim3  dimGrid_backprj(2,N,ZETA/Number_of_Devices); 
    dim3  dimBlock_backprj(M/2);
...
//         if (j==0)
//         {
//             SaveDeviceDataToFile(h_proj_single,R*Z_prj,"../projCheck.data");    
//             exit(0);
//         }  
