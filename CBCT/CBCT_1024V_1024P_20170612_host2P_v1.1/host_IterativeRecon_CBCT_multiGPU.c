...
{        
	...
	for (int j=0; j<Nviews; j=j+1)
	{
        ...
        {
            cudaSetDevice(i); 
            forward_ray_driven_3d_kernel_correction_multiGPU<<<dimGrid,dimBlock >>>(d_volume_addr[i], d_proj_portion_addr[i], h_proj_data+R*Z_prj*j+R*Z_prj/Number_of_Devices*i, sin_theta, cos_theta, i, 1); 
        }
