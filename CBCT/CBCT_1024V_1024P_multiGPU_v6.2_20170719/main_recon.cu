...
const int Iter_num = 11; 
const float Lip_con = 32.0f;

// Distances
const float DSO = 16.8e-2;
const float DOD = -41.5e-2;

// parameters for half detector offset
const float Offset =  0;

// Reconstructed volume properties
const int M = 2048;      // reconstruction volume x range  
const int N = 2048;      // reconstruction volume y range  
const int ZETA = 128;   // reconstruction volume z range  
...
const float Detector_pixel_x = 49.5e-6;  
...
const float Detector_Zmin = Detector_pixel_x*(100+0.5);

// recon volume properties
const float volumn_x = 15e-6 ;   // (in meter)
...
const float boundary_voxel_z = Detector_pixel_x*(200+0.5)*DSO/(DSO-DOD); 
...
const int Nviews = 380; 
const float us_rate = 0.500f; 
 ...
 main(int argc, char ** argv)
{
    ...
    char objectName[]="Microsphere1";
    char outputFolder[]="/Recon_Microsphere/";   
