...
const float lambda_TV = 0.01f;         //regularization parameter for the tv norm
 ...
 main(int argc, char ** argv)
{
    ...
    char directory[]="/home/huifeng/CBCT/";
    char objectName[]="SLPhantom2";
    char outputFolder[]="/Recon_Phantom_512/";   
        
	int Niter_denoise = 40;         //iter number for denoising problem
    ...
    float *F_recon;
    checkCuda( cudaMallocHost((void**)&F_recon, sizeof(float)*M*N*ZETA) ); // host pinned memory   
...
//     {
// 		printf("Error when initializing device! \n");
// 		exit(-1);
// 	}
...
// 	}      
    ...
    // load volumetric image
        ...
        Forward_3D_ray_driven_siddon(d_recon, d_proj_data);  
...
//         cout<<"Load Initial Guess Sucessfully!"<<endl;
	...
	for (int k=1;k<=Iter_num;k++)
	{
        ...
        if (FISTA==0)
        {
            ...
            Reconstruction_3D_ray_driven_CBCT(d_recon, d_proj_data, step_size); 
...
//             SaveDeviceDataToFile(d_recon,M*N*ZETA,"../Recon.dat");            
        ...
        if (FISTA==1)
        {
            ...
            cudaMemcpy(d_recon, F_Y_k, d_volume_size, cudaMemcpyHostToDevice);   
            ...
            Forward_3D_ray_driven_siddon(d_recon, d_proj_forward);
