...
float L2_norm_cpu(const float *a, const float *b, int SIZE)
{
        float sum=0;
...
double L2_norm_gpu(float *d_a, float *d_b)
{
    ...
    dim3  dimBlock_sub(R);
	dim3  dimGrid_sub(Z_prj,N_source,Nviews);     
	L2_norm_kernel<<<dimGrid_sub, dimBlock_sub>>>(d_a, d_b);  

    /* CPU summation */
//  size_t d_proj_data_size = sizeof(float)*R*Z_prj*N_source*Nviews;
// 	float *norm_result_temp = (float *)malloc(d_proj_data_size);  
// 	cudaMemcpy(norm_result_temp, d_a, d_proj_data_size, cudaMemcpyDeviceToHost);    
// 	for (int i=0;i<Z_prj*R*N_source*Nviews; i++)
...
// 	cout<<"    * L2 norm cpu test = "<<norm_result<<endl;
    ...
    getSum_kernel<<<1,1024,1024*sizeof(float)>>>(d_a,d_result,R*Z_prj*N_source*Nviews);      
...
double  TV_norm_gpu(float *d_volume)
{
    ...
    cudaMalloc((void**)&d_volume_tv, d_volume_size);
    cudaMemset(d_volume_tv, 0, d_volume_size);                
...
//         cudaMemcpy(volume_tv, d_volume_tv, d_volume_size, cudaMemcpyDeviceToHost);
...
void SaveDeviceDataToFile(float* d_data, int data_size, char filename[])
{
    ...
    cudaMemcpy(data,d_data,sizeof(float)*data_size,cudaMemcpyDeviceToHost);
