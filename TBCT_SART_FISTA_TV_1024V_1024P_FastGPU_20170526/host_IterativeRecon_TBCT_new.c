...

{
	...
	size_t d_proj_correction_size = sizeof(float)*N_source*Z_prj*R;
	cudaMalloc((void**)&d_proj_correction, d_proj_correction_size);
	cudaMemset(d_proj_correction, 0, d_proj_correction_size );       
	...
	for (int j=0; j<Nviews; j=j+1)
        {            
            ...
            backprj_ray_driven_3d_correction_new<<<dimGrid2, dimBlock2 >>>(d_volumn_weightedSum, d_volumn_length, d_proj_correction, sin_theta, cos_theta);
...
void Forward_3D_ray_driven_siddon_TBCT(float *d_volume, float *d_proj_forward)
{
	float t_theta;
    ...
    {                
        ...
        d_proj_single = d_proj_forward + Z_prj * R * N_source * j;
