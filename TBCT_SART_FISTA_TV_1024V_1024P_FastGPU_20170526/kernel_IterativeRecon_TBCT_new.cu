__global__ void forward_ray_driven_3d_correction_new(float *d_f, float *d_proj_correction, float *d_proj_data, float sin_theta, float cos_theta ,float command)
{
    /* 
     * Reference: 
...
__global__ void backprj_ray_driven_3d_correction_new(float *d_f_weightedLenSum , float *d_f_LenSum , float *d_proj_correction, float sin_theta, float cos_theta)
{
    ...
    int proj_x_idx = threadIdx.x+blockIdx.x*blockDim.x;
...
//     int proj_view_idx = blockIdx.y;
    ...
    float proj_val = d_proj_correction[R * Z_prj * proj_src_idx + R * proj_z_idx + proj_x_idx];        
