...
//     dim3  dimGrid(2, Z_prj);    
	...
	for (int j=0; j<Nviews; j=j+1)
	{
        ...
        for (int i=0; i<Number_of_Devices; i++)
        {
            cudaSetDevice(i); 
            cudaDeviceEnablePeerAccess(0,0);            
            cudaMemcpyAsync(d_proj_addr[0]+R*Z_prj/Number_of_Devices*i,d_proj_portion_addr[i],size_proj_single_portion,cudaMemcpyDefault);
        }  
        ...
        for (int i=0; i<Number_of_Devices; i++)
        {
        	cudaSetDevice(i);
            cudaDeviceEnablePeerAccess(0,0);            
            cudaMemcpyAsync(d_proj_addr[i],d_proj_addr[0],size_proj_single,cudaMemcpyDefault);
        }    
