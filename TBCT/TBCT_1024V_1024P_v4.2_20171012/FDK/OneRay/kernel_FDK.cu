...
__global__ void kernel_FDK_reconstruction(float *d_volume, float* d_volume_times, float *d_projection, float sin_theta, float cos_theta)
{
    ...
    {
        Source_z = Source_z_min + i * Source_interval;    
        s = -x*sin_theta + y*cos_theta;         
        ksi=DSO*(z-Source_z)/(DSO-s);
        ksi=(ksi*(DSO-DOD)/DSO+Source_z - Detector_Zmin) /Detector_pixel_x;  
        if ((0<=ksi)&&(ksi+1<Z_prj))
        {
            if (idx_source==-10)
                idx_source=i;
            else if (abs(Source_z -z)<abs(Source_z_min+idx_source*Source_interval -z))
                idx_source=i;
        }
    }
    Source_z = Source_z_min + idx_source * Source_interval;  
    ...
    d_volume[idx_voxel]+=temp1*temp2 * (us_rate*PI/180.0f) * 360.0f/(us_rate*Nviews);            
