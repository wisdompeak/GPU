...
__global__ void backprj_ray_driven_3d_kernel_multiGPU(float* d_volume_weightedSum, float *d_volume_length,  float *d_proj_correction, float sin_theta, float cos_theta, int subVolIdx)
{    
    /* 
    ...
    int Idx_voxel_x = blockDim.x*(blockIdx.x / N) + threadIdx.x;
    int Idx_voxel_y = blockIdx.x % N;
    int Idx_voxel_z = blockIdx.y;
...
//     float tao_m1 = atan( (float(R)*Detector_pixel_x/2.0f-abs(Offset)) / DSO); 
	...
	if ( (Idx_voxel_x-(M*0.5f-0.5f)-M_Offset)*volumn_x*(Idx_voxel_x-(M*0.5f-0.5f)-M_Offset)*volumn_x 
            +  (Idx_voxel_y-(N*0.5f-0.5f))*volumn_y*(Idx_voxel_y-(N*0.5f-0.5f))*volumn_y 
