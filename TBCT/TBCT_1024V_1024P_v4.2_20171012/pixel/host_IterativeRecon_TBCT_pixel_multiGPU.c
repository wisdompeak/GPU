...
{        
    ...
    for (int i=0; i<Number_of_Devices; i++)
    {
       ...
       cudaMemset(d_proj_sumLen_addr[i],0,size_proj_single);
       cudaMalloc((void**)&d_proj_weightedLen_addr[i], size_proj_single); 
       cudaMemset(d_proj_weightedLen_addr[i],0,size_proj_single);
       ...
       cudaMalloc((void**)&d_volume_weightedSum_addr[i], size_volume_portion); 
       cudaMemset(d_volume_weightedSum_addr[i],0,size_volume_portion);
       cudaMalloc((void**)&d_volume_length_addr[i], size_volume_portion); 
       cudaMemset(d_volume_length_addr[i],0,size_volume_portion);       
    }             
    ...
    dim3  dimGrid_forward(2, Z_prj,N_source);
    dim3  dimBlock_forward(R/2);    
    
	//setup execution parameters for backprojection
	dim3  dimGrid_backprj(1, Z_prj,N_source);
    dim3  dimBlock_backprj(R/1); 
    
    dim3  dimGrid_V(N,ZETA/Number_of_Devices);
    dim3  dimBlock_V(M);    
	...
	for (int j=0; j<Nviews; j=j+1)
	{
        ...
        cudaSetDevice(Default_GPU); 
        kernel_divide_proj<<<dimGrid_forward,dimBlock_forward>>>(h_proj_single, h_proj_data+Z_prj*R*N_source*j, h_proj_sumLen, h_proj_weightedLen);
        cudaDeviceSynchronize(); 
       
        /**** for test use ****/
//         if (j==0)
//         {
//             SaveDeviceDataToFile(h_proj_sumLen,R*Z_prj*N_source,"../projCheck.data");        
//             exit(0);
//         }  
        ...
        {              
            cudaSetDevice(i);
            backprj_ray_driven_3d_kernel_multiGPU<<<dimGrid_backprj,dimBlock_backprj >>>(d_volume_weightedSum_addr[i], d_volume_length_addr[i], d_proj_addr[i], sin_theta, cos_theta, i);             
        }                        
        for (int i=0; i<Number_of_Devices; i++)
        {
            cudaSetDevice(i);
        ...
        {              
            cudaSetDevice(i);
            update<<< dimGrid_V,dimBlock_V >>>(d_volume_addr[i], d_volume_weightedSum_addr[i], d_volume_length_addr[i], beta_temp);
        }                        
        for (int i=0; i<Number_of_Devices; i++)
        {
            cudaSetDevice(i);
...
{        
	...
	for (int i=0; i<Number_of_Devices; i++)
    {              
        cudaMemcpyAsync(h_volume_backprj+M*N*ZETA/Number_of_Devices*i,d_volume_weightedSum_addr[i],size_volume_portion,cudaMemcpyDefault);
	}                        
    cudaDeviceSynchronize();
