...
void Reconstruction_3D_ray_driven_CBCT(float *h_volume, float *d_proj_data, float beta_temp)
{        
    // set Timer starter
	...
	for (int j=0; j<Nviews; j=j+1)
	{
        ...
        cudaMemcpyAsync(h_proj_single, d_proj_data + Z_prj*R*j, size_proj_single, cudaMemcpyDefault);
        ...
        {
            cudaSetDevice(i); 
            forward_ray_driven_3d_kernel_correction_multiGPU<<<dimGrid,dimBlock >>>(d_volume_addr[i], d_proj_portion_addr[i], h_proj_single+R*Z_prj/Number_of_Devices*i, sin_theta, cos_theta, i, 1); 
        }
        ...
        {              
            cudaSetDevice(i);
            backprj_ray_driven_3d_kernel_multiGPU<<<dimGrid_backprj, dimBlock_backprj>>>(h_volume+M*N*ZETA/Number_of_Devices*i, d_proj_addr[i], beta_temp, sin_theta, cos_theta, i, 1); 
        }                        
        for (int i=0; i<Number_of_Devices; i++)
        {
            cudaSetDevice(i);
...
void Forward_3D_ray_driven_siddon(float *h_volume, float *d_proj_data)
{
...
//     }   
    ...
    {                
        ...
        cudaSetDevice(Default_GPU);
        cudaMemcpyAsync(d_proj_data + Z_prj*R*j,h_proj_single,size_proj_single,cudaMemcpyDefault);                       
	}        
